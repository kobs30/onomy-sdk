...
ldflags = -X github.com/onomyprotocol/onomy-sdk/version.Name=sim \
		  -X github.com/onomyprotocol/onomy-sdk/version.AppName=simd \
...
ifeq (cleveldb,$(findstring cleveldb,$(COSMOS_BUILD_OPTIONS)))
  ldflags += -X github.com/onomyprotocol/onomy-sdk/types.DBBackend=cleveldb
endif
ifeq (badgerdb,$(findstring badgerdb,$(COSMOS_BUILD_OPTIONS)))
  ldflags += -X github.com/onomyprotocol/onomy-sdk/types.DBBackend=badgerdb
  BUILD_TAGS += badgerdb
endif
# handle rocksdb
ifeq (rocksdb,$(findstring rocksdb,$(COSMOS_BUILD_OPTIONS)))
  CGO_ENABLED=1
  BUILD_TAGS += rocksdb
  ldflags += -X github.com/onomyprotocol/onomy-sdk/types.DBBackend=rocksdb
endif
# handle boltdb
ifeq (boltdb,$(findstring boltdb,$(COSMOS_BUILD_OPTIONS)))
  BUILD_TAGS += boltdb
  ldflags += -X github.com/onomyprotocol/onomy-sdk/types.DBBackend=boltdb
endif
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/onomyprotocol/onomy-sdk/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.ochaind/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.ochaind/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.ochaind/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.ochaind/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -path "./tests/mocks/*" -not -name '*.pb.go' | xargs goimports -w -local github.com/onomyprotocol/onomy-sdk
.PHONY: format
...
devdoc-init:
	$(DOCKER) run -it -v "$(CURDIR):/go/src/github.com/onomyprotocol/onomy-sdk" -w "/go/src/github.com/onomyprotocol/onomy-sdk" tendermint/devdoc echo
	# TODO make this safer
	$(call DEVDOC_SAVE)

devdoc:
	$(DOCKER) run -it -v "$(CURDIR):/go/src/github.com/onomyprotocol/onomy-sdk" -w "/go/src/github.com/onomyprotocol/onomy-sdk" devdoc:local bash
...
## use of `sed` in order to build properly with onomy-sdk's proto file layout
