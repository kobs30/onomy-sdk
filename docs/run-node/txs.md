...
::: warning
Please note that signing a transaction with multiple signers or with a multisig account, where at least one signer uses `SIGN_MODE_DIRECT`, is not yet possible. You may follow [this Github issue](https://github.com/onomyprotocol/onomy-sdk/issues/8141) for more info.
:::
...
Populating the `TxBuilder` can be done via its [methods](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/client/tx_config.go#L32-L45):
...
We set encoding config to use Protobuf, which will use `SIGN_MODE_DIRECT` by default. As per [ADR-020](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/docs/architecture/adr-020-protobuf-transaction-encoding.md), each signer needs to sign the `SignerInfo`s of all other signers. This means that we need to perform two steps sequentially:
