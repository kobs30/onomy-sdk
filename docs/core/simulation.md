...
On the SDK, this functionality is provided by the[`SimApp`](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0/simapp/app.go), which is a
`Baseapp` application that is used for running the [`simulation`](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0/x/simulation) module.
...
Check the full list of flags [here](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0/simapp/config.go#L32-L55).
...
3. From a `params.json` file where the initial state is pseudo-randomly generated but the module and simulation parameters can be provided manually.
   ...
   The list of available parameters are listed [here](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0/x/simulation/params.go#L44-L52).
...
check the SDK [Makefile](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0/Makefile#L251-L287).

```bash
 $ go test -mod=readonly github.com/onomyprotocol/onomy-sdk/simapp \
...
- Try adding logs to operations that are not logged. You will have to define a
  [Logger](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0/x/staking/keeper/keeper.go#L66-L69) on your `Keeper`.
