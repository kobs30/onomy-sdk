...
我们将“服务提供商”定义为可以为最终用户提供服务的实体，这些实体涉及与基于 Onomy-SDK 的区块链（包括 Cosmos Hub）的某种形式的交互。更具体地说，本文档将集中于与 token 的交互。

本节不涉及想要提供[轻客户端](https://github.com/onomyprotocol/onomy-sdk/tree/master/docs/interfaces/lite)功能的钱包开发者。服务提供商将作为最终用户的区块链的可信接入点。
...
```bash
ochaincli keys add <your_key_name>
```
...
```bash
ochaincli account <YOUR_ADDRESS>
```
...
```bash
ochaincli tx send <from_key_or_address> <to_address> <amount> \
...
```bash
ochaincli rest-server --node=<full_node_address:full_node_port>
```
...
每个 Cosmos SDK 交易都有一个规范的 JSON 描述。 `ochaincli`和 REST 接口为交易提供规范的 JSON 描述，“广播”功能将提供紧凑的 Amino（类似 protobuf 的格式）编码转换。
