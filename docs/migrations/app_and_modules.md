...
In Onomy-SDK we manage the project using Makefile. Your own app can use a similar Makefile to the [Cosmos SDK's one](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/Makefile). More specifically, below are some _make_ commands that might be useful for your own app, related to the introduction of Protocol Buffers:
...
- // https://github.com/onomyprotocol/onomy-sdk/blob/v0.39.2/x/bank/internal/types/msgs.go#L12-L16
...
+ // https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/proto/cosmos/auth/v1beta1/auth.proto#L13-L25
+ message BaseAccount {
+  string              address = 1;
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/x/auth/types/auth.pb.go#L28-L36
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/proto/cosmos/bank/v1beta1/tx.proto#L10-L31
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/proto/cosmos/bank/v1beta1/query.proto#L12-L23
...
Cosmos SDK v0.40 also comes with an efficient pagination, it now uses `Prefix` stores to make queries. There are 2 helpers for pagination, [`Paginate`](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/types/query/pagination.go#L40-L42), [`FilteredPaginate`](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/types/query/filtered_pagination.go#L9-L17).
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/x/bank/module.go#L99-L103
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/x/bank/module.go#L69-L72
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/x/bank/types/codec.go#L21-L34
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/x/bank/module.go#L35-L38

### CLI

Each modules may optionally expose CLI commands, and some changes are needed in these commands.

First, `context.CLIContext` is renamed to `client.Context` and moved to `github.com/onomyprotocol/onomy-sdk/client`.
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/x/bank/client/cli/query.go#L66-L94
...
| ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `alias.go` file                                                               | Removed                                                                                              | `alias` usage is removed, please see [#6311](https://github.com/onomyprotocol/onomy-sdk/issues/6311) for details.                                                                                                           |
...
| `Route() string`                                                              | `Route() sdk.Route`                                                                                  | For legacy handlers, return type of `Route()` method is changed from `string` to `"github.com/onomyprotocol/onomy-sdk/types".Route`. It should return a `NewRoute()` which includes `RouterKey` and `NewHandler` as params. |
...
For a reference implementation used for demo purposes, you can refer to the SDK's [SimApp](https://github.com/onomyprotocol/onomy-sdk/tree/v0.40.0-rc6/simapp) for your app's migration. The most important changes are described in this section.

### Creating Codecs

With the introduction of Protobuf, each app needs to define the encoding library (Amino or Protobuf) to be used throughout the app. There is a central struct, [`EncodingConfig`](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/simapp/params/encoding.go#L9-L11), which defines all information necessary for codecs. In your app, an example `EncodingConfig` with Protobuf as default codec might look like:
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/simapp/app.go#L146-L153
...
- the [Tx Service](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/proto/cosmos/tx/v1beta1/service.proto), to perform operations on transactions,
- the [Tendermint service](https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/proto/cosmos/base/tendermint/v1beta1/query.proto), to have a more idiomatic interface to the [Tendermint RPC](https://docs.tendermint.com/master/rpc/).
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/simapp/app.go#L577-L585
...
+++ https://github.com/onomyprotocol/onomy-sdk/blob/v0.40.0-rc6/simapp/app.go#L555-L575
